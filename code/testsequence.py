...
class Testsequence:
    ...
    def __init__(self, testname, epochs):
        self.models = []
        self.text_file = open("logs/test_" + testname + ".log", "w")
    ...
    def add_model(self, numconvlayer, numfilters, numdense, numnodes, pooling):
        ...
        for i in range(1,numdense):
            model.add(Dense(numnodes, activation='relu', kernel_initializer='he_uniform'))
    ...
    def trainall(self, traingenerator, valgenerator, safemodel):
        ...
        for model in self.models:
            ...
            #plt.show()
            plt.savefig("plots/model" + str(i) + ".pdf")
            plt.close()
            self.reset_keras()
